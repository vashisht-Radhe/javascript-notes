...
Variables in js are `containers used for storing data values`. we can think of them as named storage locations in memory that can be accessed and manipulated throughout a program's execution.

*Variables are declared using one of three keywords: `var`, `let`, or `const`.*
